...
{ 
	...
	{ 
		...
		static long SectionCritique_RECURSIF = 0;
		...
		bool supprimer_Processus(unsigned long pid, __attribute__((unused)) bool force)
		{
			// Cette fonction permet de signaler l'arret d'un processus
			//  et donc de tous ses threads
			//  si force=true 
			long compteur_thread = 0;
			...
			else
			{
				...
				if(*Liste_Processus[pid].Nom_Processus != 0)
		...
		unsigned long check_Thread_zombie(__attribute__((unused)) bool liberer, __attribute__((unused)) bool debug)
		{
		...
		bool free_Thread_zombie(__attribute__((unused)) unsigned long tid)
		{
		...
		bool supprimer_Thread(unsigned long tid, bool force)
		{
			...
			if(force == true)
			{
				...
				if(*Liste_Threads[tid].Nom_Thread != 0)
		...
		void Interruption_Timer(__attribute__((unused)) long Priorite)
		{	
