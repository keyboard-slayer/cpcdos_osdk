/*
        ...
        Description
                Module permettant de creer un serveur TCP/UDP multi-client
   multithread multi I/O
        ...
        Mise a jour
                ...
                07-12-2017	: AMELIORATION du code en suivant une procedure
   sticte de GCC 11-04-2017	: ADAPTATION pour creation de serveur d'ECHO.
                21-02-2017	: DEBUG ajout info port
                14-02-2017	: Revue des includes + Finitions
                13-02-2017	: Ajout du controle d'envoie (#CLT# et #TOUT#)
   pour choisir en envoie general ou cible 07-02-2017	: Ajout des
   fonctionnalites de controle de buffer
...
long Demarrer_serveur(unsigned long _NumPort, long _NombreClients,
                      unsigned long __NumeroID, long __TYPE_SERVEUR) {

  // Cette fonction permet de creer un serveur TCP
  //  _TYPE_SERVEUR 	= TCP:1 / UDP:2 / CCP:3 / TELNET:4 / ECHO tcp:5 / ECHO
  //  udp:6 NumPort			= Numero de port
  //	_NumeroID		= Numero d'itentification unique associe a un
  // processus
  ...
  std::string MSG_Refus_Full =
      "[EN] Server is full / [FR] Le serveur est plein";
  ...
  cpinti_dbg::CPINTI_DEBUG(
      "Creation d'un serveur '" + _TYPE_SERVEUR_STR + "' port:" + NumPort_STR +
          " " + NombreClients_STR + " client(s) max.",
      "Creating '" + _TYPE_SERVEUR_STR + "' server, port:" + NumPort_STR + " " +
          NombreClients_STR + " client(s) max.",
      "SRV:" + NumPort_STR, "Demarrer_serveur()", Ligne_saute, Alerte_ok,
      Date_avec, Ligne_r_normal);

  /******************************************************************/
  /********************** CREATION D'UN SOCKET **********************/
  /******************************************************************/

  cpinti_dbg::CPINTI_DEBUG("Configuration du socket ", "Socket configuration ",
                           "SRV:" + NumPort_STR, "Demarrer_serveur()",
                           Ligne_reste, Alerte_action, Date_avec,
                           Ligne_r_normal);

  // Creer le socket du serveur
  if ((SocketReseau = socket(AF_INET, SOCK_STREAM, 0)) == 0) {
    // Stocker l'erreur dans un attribut membre de la classe qu'on pourrai
    // recuperer
    Erreur_STR = std::string(strerror(errno));
    cpinti_dbg::CPINTI_DEBUG(
        "[ERREUR] Impossible de creer le socket. Raison:'" + Erreur_STR + "'",
        "[ERROR] Unable to create socket. Reason:'" + Erreur_STR + "'",
        "SRV:" + NumPort_STR, "Demarrer_serveur()", Ligne_saute, Alerte_erreur,
        Date_avec, Ligne_r_normal);
    return SRV_ERR_INIT_SOCK;
  }

  cpinti_dbg::CPINTI_DEBUG(".", ".", "", "", Ligne_reste, Alerte_action,
                           Date_sans, Ligne_r_normal);
  ...
  if (setsockopt(SocketReseau, SOL_SOCKET, SO_REUSEADDR, (char *)&opt,
                 sizeof(opt)) < 0) {
    // Stocker l'erreur dans un attribut membre de la classe qu'on pourrai
    // recuperer
    Erreur_STR = std::string(strerror(errno));
    cpinti_dbg::CPINTI_DEBUG(
        "[ERREUR] Impossible de configurer 'SO_REUSEADDR' sur le socket . "
        "Raison:'" +
            Erreur_STR + "'",
        "[ERROR] Unable to configure 'SO_REUSEADDR' on the socket. Reason:'" +
            Erreur_STR + "'",
        "SRV:" + NumPort_STR, "Demarrer_serveur()", Ligne_saute, Alerte_erreur,
        Date_avec, Ligne_r_normal);
    return SRV_ERR_INIT_SOCK;
  ...
  cpinti_dbg::CPINTI_DEBUG(".", ".", "", "", Ligne_reste, Alerte_action,
                           Date_sans,
                           Ligne_r_normal); // Accepter les multiples connexions
  if (setsockopt(SocketReseau, SOL_SOCKET, SO_REUSEADDR, (char *)&opt,
                 sizeof(opt)) < 0) {
    // Stocker l'erreur dans un attribut membre de la classe qu'on pourrai
    // recuperer
  ...
  if (setsockopt(SocketReseau, SOL_SOCKET, CPC_SO_RCVTIMEO, (char *)&TempsMAX,
                 sizeof(TempsMAX)) == -1) {
    // Stocker l'erreur dans un attribut membre de la classe qu'on pourrai
    // recuperer
    Erreur_STR = std::string(strerror(errno));
    cpinti_dbg::CPINTI_DEBUG(
        "[ERREUR] Impossible de configurer 'CPC_SO_RCVTIMEO' sur le socket. "
        "Raison:'" +
            Erreur_STR + "'",
        "[ERROR] Unable to configure 'CPC_SO_RCVTIMEO' on the socket. "
        "Reason:'" +
            Erreur_STR + "'",
        "SRV:" + NumPort_STR, "", Ligne_saute, Alerte_erreur, Date_avec,
        Ligne_r_normal);
    return SRV_ERR_INIT_SOCK;
  }

  cpinti_dbg::CPINTI_DEBUG(" [OK]", " [OK]", "", "", Ligne_saute, Alerte_ok,
                           Date_sans, Ligne_r_normal);
  ...
  cpinti_dbg::CPINTI_DEBUG("Binding socket ", "Socket binding ",
                           "SRV:" + NumPort_STR, "Demarrer_serveur()",
                           Ligne_reste, Alerte_action, Date_avec,
                           Ligne_r_normal);
  ...
  if (bind(SocketReseau, (struct sockaddr *)&Sock_Adresse,
           sizeof(Sock_Adresse)) < 0) {
    Erreur_STR = std::string(strerror(errno));
    cpinti_dbg::CPINTI_DEBUG(
        "[ERREUR] Impossible de binder le socket. Raison:'" + Erreur_STR + "'",
  ...
  if (listen(SocketReseau, NombreClients + 4) < 0) {
    Erreur_STR = std::string(strerror(errno));
    cpinti_dbg::CPINTI_DEBUG(
        "[ERREUR] Impossible de d'ecouter le socket. Raison:'" + Erreur_STR +
            "'",
...

  {
    ...
    cpinti_dbg::CPINTI_DEBUG(
        "Serveur demarre. Attente des connexions client (" + NbClientCO_STR +
            "/" + NombreClients_STR + ")",
        "Server started. Waiting client connections (" + NbClientCO_STR + "/" +
            NombreClients_STR + ")",
        "SRV:" + NumPort_STR, "", Ligne_saute, Alerte_ok, Date_avec,
        Ligne_r_normal);
    ...
    while (EN_VIE) {
      ...
      STACK_MEMOIRE_STR =
          cpinti::cpinti_GEST_BUFF(NumPort, _STACK_EXTRACT_POUR_SERVEUR, "");
      ...
      if (STACK_MEMOIRE_STR.length() > 0) {
        ...
        if (BUFFER == "#STOP") {
          cpinti_dbg::CPINTI_DEBUG(
              "Fermeture du serveur a la demande du noyau ...",
              "Closing server at the request of the kernel ...",
              "SRV:" + NumPort_STR, "", Ligne_saute, Alerte_action, Date_avec,
              Ligne_r_normal);
          ...
          for (i = 0; i < NombreClients; i++) {
            if (client_socket[i] > 0) {
              send(client_socket[i], "Server is down", BUFFER.length(),
                   CPC_MSG_DONTWAIT);
              NombreEnvoie++;
            }
          }
          ...
          for (i = 0; i < NombreClients; i++) {
            if (client_socket[i] > 0) {
              ...
              cpinti_dbg::CPINTI_DEBUG(
                  "Deconnexion du client Socket#" + index_STR + "/" +
                      NombreEnvoie_STR + " [" + client_IP[i] + ":" + port_STR +
                      "] ...",
                  "Disconnecting client Socket#" + index_STR + "/" +
                      NombreEnvoie_STR + " [" + client_IP[i] + ":" + port_STR +
                      "] ...",
                  "SRV:" + NumPort_STR, "", Ligne_saute, Alerte_action,
                  Date_avec, Ligne_reste);

              Fermer_socket(client_socket[i]);

              cpinti_dbg::CPINTI_DEBUG(" [OK]", " [OK]", "", "", Ligne_saute,
        ...
        if (BUFFER.find("#ETC ") != std::string::npos) {
          ...
          std::string Client_PORT_STR =
              BUFFER.substr(posPORT + 1, posFINPORT - (posPORT + 1));

          // Recuperer le numero de socket
          std::string Client_SOCK_STR =
              BUFFER.substr(posFINPORT + 4, posFINSOCK - (posFINPORT + 4));

          // A afficher seulement si le debogeur reseau est actif
          cpinti_dbg::CPINTI_DEBUG(
              "Echo [" + Client_IP_STR + ":" + Client_PORT_STR + "] Socket #" +
                  Client_SOCK_STR + " ... ",
              "Echo [" + Client_IP_STR + ":" + Client_PORT_STR + "] Socket #" +
                  Client_SOCK_STR + " ... ",
              "SRV:" + NumPort_STR, "", Ligne_saute, Alerte_action, Date_avec,
          ...
          for (i = 0; i < NombreClients; i++) {
            if (client_socket[i] > 0) {
              send(client_socket[i], BUFFER.c_str(), BUFFER.length(),
                   CPC_MSG_DONTWAIT);
              NombreEnvoie++;
            }
          }
          ...
          cpinti_dbg::CPINTI_DEBUG("Les donnees (" + BUFFER_length_STR +
                                       " octets) ont ete envoyes a " +
                                       NombreEnvoie_STR + " client(s) [OK]",
                                   "Data (" + BUFFER_length_STR +
                                       " bytes) has been sent at " +
                                       NombreEnvoie_STR + " client(s) [OK]",
                                   "SRV:" + NumPort_STR, "", Ligne_saute,
                                   Alerte_ok, Date_avec, Ligne_r_normal);
        }
          ...
          if (Client_SOCK > 0)
            ...
            for (i = 0; i < NombreClients; i++) {
              ...
              if (client_socket[i] == Client_SOCK) {
                ...
                cpinti_dbg::CPINTI_DEBUG(
                    "Envoi de " + BUFFER_length_STR + " octets a [" +
                        client_IP[i] + ":" + client_PORT_STR + "] Socket #" +
                        Client_SOCK_STR + " ... ",
                    "Sending " + BUFFER_length_STR + " bytes to [" +
                        client_IP[i] + ":" + client_PORT_STR + "] Socket #" +
                        Client_SOCK_STR + " ... ",
                    "SRV:" + NumPort_STR, "", Ligne_reste, Alerte_action,
                    Date_avec, Ligne_r_normal);
      ...
      if ((Resultat < 0) && (errno != EINTR)) {
        Erreur_STR = std::string(strerror(errno));
        cpinti_dbg::CPINTI_DEBUG(
            "[AVERTISSEMENT] Probleme de socket. Raison:'" + Erreur_STR + "'",
            "[WARNING] Socket problem. Reason:'" + Erreur_STR + "'",
            "SRV:" + NumPort_STR, "Demarrer_serveur()", Ligne_saute,
            Alerte_avertissement, Date_avec, Ligne_r_normal);
      }

      // Il y a eu quelque chose! Oh My GOD!
      if (FD_ISSET(SocketReseau, &FD_socket)) {
        if ((New_Socket = accept(SocketReseau, (struct sockaddr *)&Sock_Adresse,
                                 (socklen_t *)&Taille_Adresse)) < 0) {
          Erreur_STR =
              std::string(strerror(errno)) +
              std::string(" errno(" + std::to_string(errno) + ") srvsck(" +
                          std::to_string(SocketReseau) + ")");
          cpinti_dbg::CPINTI_DEBUG(
              "[ERREUR] Impossible d'accepter le client. Raison:'" +
                  Erreur_STR + "'",
              "[ERROR] Unable to accept client. Reason:'" + Erreur_STR + "'",
              "SRV:" + NumPort_STR, "Demarrer_serveur()", Ligne_saute,
              Alerte_erreur, Date_avec, Ligne_r_normal);
          Fermer_socket(SocketReseau);
        ...
        if (NbClientCO >= NombreClients) {
          Erreur_STR = std::string(strerror(errno));
          cpinti_dbg::CPINTI_DEBUG("Connexion refuse [" + IP_STR + ":" +
                                       PORT_STR + "] Socket #" + New_Socket_STR,
                                   "Refused connection [" + IP_STR + ":" +
                                       PORT_STR + "] Socket #" + New_Socket_STR,
                                   "SRV:" + NumPort_STR, "", Ligne_saute,
                                   Alerte_avertissement, Date_avec,
                                   Ligne_r_normal);
          ...
          cpinti_dbg::CPINTI_DEBUG("Connexion accepte [" + IP_STR + ":" +
                                       PORT_STR + "] Socket #" + New_Socket_STR,
                                   "Accepted connection [" + IP_STR + ":" +
                                       PORT_STR + "] Socket #" + New_Socket_STR,
                                   "SRV:" + NumPort_STR, "", Ligne_reste,
                                   Alerte_ok, Date_avec, Ligne_r_normal);
          ...
          // Envoyer un message
          // if( send(New_Socket, message, strlen(message), 0) != (ssize_t)
          // strlen(message) )
          // {
          // Erreur_STR = std::string(strerror(errno));
          // cpinti_dbg::CPINTI_DEBUG(" [AVERTISSEMENT] Probleme lors de l'envoi
          // du message. Raison:'" + Erreur_STR + "'", " [WARNING] Problem
          // during sending. Reason:'" + Erreur_STR + "'",
          // "SRV:" + NumPort_STR, "Demarrer_serveur()", Ligne_saute,
          // Alerte_avertissement, Date_avec, Ligne_r_normal);
          // }
        }
      ...
      for (i = 0; i < NombreClients; i++) {
        ...
        if (FD_ISSET(SocketClient, &FD_socket)) {
          ...
          std::string IP_STR = std::string(inet_ntoa(Sock_Adresse.sin_addr));
          std::string PORT_STR =
              std::to_string((int)ntohs(Sock_Adresse.sin_port));

          // Attendre que le socket se ferme, autrement receptionner les
          // messages
          if ((TailleLue = read(SocketClient, buffer, TailleBuffer)) == 0) {
            // Un client s'est barr�.. On le ferme!
            getpeername(SocketClient, (struct sockaddr *)&Sock_Adresse,
            ...
            cpinti_dbg::CPINTI_DEBUG(
                "Deconnexion [" + IP_STR + ":" + PORT_STR + "] Socket #" +
                    New_Socket_STR + " index " + i_STR + " (" + NbClientCO_STR +
                    "/" + NombreClients_STR + ")",
                "Disconnection [" + IP_STR + ":" + PORT_STR + "] Socket #" +
                    New_Socket_STR + " index " + i_STR + " (" + NbClientCO_STR +
                    "/" + NombreClients_STR + ")",
                "SRV:" + NumPort_STR, "", Ligne_reste, Alerte_avertissement,
                Date_avec, Ligne_r_normal);
            ...
            if (_TYPE_SERVEUR == 1) /* TCP */
              cpinti::cpinti_GEST_BUFF(NumPort, _STACK_STOCKER_POUR_CPCDOS,
                                       "#TCP " + IP_STR + ":" + PORT_STR +
                                           " TCP#" + New_Socket_STR + "=" +
                                           std::string(buffer));
              ...
              cpinti::cpinti_GEST_BUFF(NumPort, _STACK_STOCKER_POUR_SERVEUR,
                                       "#ETC " + IP_STR + ":" + PORT_STR +
                                           " TCP#" + New_Socket_STR + "=" +
                                           std::string(buffer));
