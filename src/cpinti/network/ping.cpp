/*
        ...
        Mise a jour
                ...
                29-01-2018	: CORRECTION du calcul du temps qui etait bas�
   sur la trame re�ue et non la trame d'envoie
                07-12-2017	: AMELIORATION du code en suivant une procedure
   sticte de GCC 05-01-2017  : Correction du probleme de reception de la trame
   (return mal place) : Correction des 4 octets en trop dans la trame
                                                memcpy(&PAQUET_ICMP[TAILLE_ICMP_HDR+TAILLE_Temps_Debut+TAILLE_PAQUET_ENVOYE##-4##]
   --> "-4" supprime!
...
long ping(const char *AdresseIP, const char *Message, __attribute__((unused)) long _Timeout) {

  std::string AdresseIP_STR = std::string(AdresseIP);
  cpinti_dbg::CPINTI_DEBUG(
      "Ping vers " + AdresseIP_STR, "Ping to " + AdresseIP_STR, "net_ping",
      "ping()", Ligne_saute, Alerte_action, Date_avec, Ligne_r_normal);

  cpinti_dbg::CPINTI_DEBUG(" * Configuration du socket reseau... ",
                           " * Configuration network socket... ", "net_ping",
                           "ping()", Ligne_reste, Alerte_action, Date_avec,
                           Ligne_r_normal);
  ...
  if (SocketReseau < 0) {
    ...
    Erreur_STR = std::string(strerror(errno));
    cpinti_dbg::CPINTI_DEBUG(
        " [ERREUR] Impossible de creer un socket RECEIVE. Raison:'" +
            Erreur_STR + "'",
        " [ERROR] Unable to create socket. Reason:'" + Erreur_STR + "'",
        "net_ping", "ping()", Ligne_saute, Alerte_erreur, Date_avec,
        Ligne_r_normal);
  ...
  /******************** CONFIGURATION DU SOCKET *********************/
  /******************************************************************/
  Sock_Option = CPC_SO_RCVTIMEO; // Configurer la reception

  if (setsockopt(SocketReseau, Sock_Niveau, Sock_Option, Sock_Option_VAL,
                 Sock_Option_TAILLE) == -1) {

    Erreur_STR = std::string(strerror(errno));
    cpinti_dbg::CPINTI_DEBUG(
        " [ERREUR] Impossible de configurer le socket RECEIVE. Raison:'" +
            Erreur_STR + "'",
  ...
  Sock_Option = CPC_SO_SNDTIMEO; // Configurer l'envoi

  if (setsockopt(SocketReseau, Sock_Niveau, Sock_Option, Sock_Option_VAL,
                 Sock_Option_TAILLE) == -1) {
    Erreur_STR = std::string(strerror(errno));
    cpinti_dbg::CPINTI_DEBUG(
        " [ERREUR] Impossible de configurer le socket SEND. Raison:'" +
            Erreur_STR + "'",
  ...
  int flag = 1;
  if (setsockopt(SocketReseau, Sock_Niveau, TCP_NODELAY, (char *)&flag,
                 sizeof(int)) == -1) {
    Erreur_STR = std::string(strerror(errno));
    cpinti_dbg::CPINTI_DEBUG(
        " [ERREUR] Impossible de configurer le socket. Raison:'" + Erreur_STR +
            "'",
  ...
  cpinti_dbg::CPINTI_DEBUG("[OK] Socket : " + std::to_string(SocketReseau),
                           "[OK] Socket : " + std::to_string(SocketReseau), "",
  ...
  /******************************************************************/
  cpinti_dbg::CPINTI_DEBUG(
      " * Resolution DNS si possible de '" + AdresseIP_STR + "'... ",
      " * DNS resolution if possible '" + AdresseIP_STR + "'... ", "net_ping",
      "ping()", Ligne_reste, Alerte_action, Date_avec, Ligne_r_normal);
  ...
  if (AdresseIP_STR == "#ERR") {
    cpinti_dbg::CPINTI_DEBUG(
        " [ERREUR] Impossible de resoudre le DNS '" + AdresseIP_STR +
            "'. Raison:'" + Erreur_STR + "'",
        " [ERROR] Unable to resolve DNS '" + AdresseIP_STR + "'. Reason:'" +
            Erreur_STR + "'",
  ...
  cpinti_dbg::CPINTI_DEBUG("Binding socket ", "Socket binding ", "net_ping",
                           "ping()", Ligne_reste, Alerte_action, Date_avec,
                           Ligne_r_normal);
  ...
  bcopy((char *)Sock_hostent->h_addr, (char *)&Sock_sockaddr.sin_addr.s_addr,
  ...
  cpinti_dbg::CPINTI_DEBUG("Preparation de la trame ICMP... ",
                           "Preparation of the ICMP frame... ", "net_ping",
                           "ping()", Ligne_reste, Alerte_action, Date_avec,
                           Ligne_r_normal);
  ...
  size_t TAILLE_ICMP_HDR, TAILLE_PAQUET_ENVOYE, TAILLE_Temps_Debut,
      TAILLE_Finale;
  ...
  TRAME_ICMP->icmp_id = (rand() % 65500 + 1) &
                        0xFFFF; // 6 501 possibilites c'est deja pas mal hein!
  ...
  TAILLE_Finale = TAILLE_ICMP_HDR + TAILLE_PAQUET_ENVOYE + TAILLE_Temps_Debut +
                  Taille_Message;

  // Packager l'entete, la requete et le message
  memcpy(&PAQUET_ICMP[TAILLE_ICMP_HDR], &Temps_Depart, TAILLE_Temps_Debut);
  memcpy(&PAQUET_ICMP[TAILLE_ICMP_HDR + TAILLE_Temps_Debut],
         &Nombre_paquets_envoyes, TAILLE_PAQUET_ENVOYE);
  memcpy(
      &PAQUET_ICMP[TAILLE_ICMP_HDR + TAILLE_Temps_Debut + TAILLE_PAQUET_ENVOYE],
      Message, Taille_Message);

  // Generer un checksum de la trame pour comparer au retour
  TRAME_ICMP->icmp_cksum =
      cpinti::Func_Cpinti::Generer_CheckSum((void *)TRAME_ICMP, TAILLE_Finale);

  std::string TAILLE_STR = std::to_string(TAILLE_Finale);
  std::string SUM_STR = std::to_string((int)TRAME_ICMP->icmp_cksum);

  cpinti_dbg::CPINTI_DEBUG(
      "[OK] --> Taille:" + TAILLE_STR + " [chksum:" + SUM_STR + "]",
      "[OK] --> Size:" + TAILLE_STR + " [chksum:" + SUM_STR + "]", "", "",
      Ligne_saute, Alerte_ok, Date_sans, Ligne_r_normal);

  cpinti_dbg::CPINTI_DEBUG("Envoi de la trame ICMP ", "Sending ICMP frame ",
                           "net_ping", "ping()", Ligne_reste, Alerte_action,
  ...
  Resultat =
      sendto(SocketReseau, TRAME_ICMP, (size_t)TAILLE_Finale, CPC_MSG_DONTWAIT,
             (struct sockaddr *)&Sock_sockaddr, sizeof(Sock_sockaddr));

  cpinti_dbg::CPINTI_DEBUG("(" + std::to_string(Resultat) + "/" +
                               std::to_string(TAILLE_Finale) + " octets) ... ",
                           "(" + std::to_string(Resultat) + "/" +
                               std::to_string(TAILLE_Finale) + " bytes) ... ",
                           "", "", Ligne_reste, Alerte_action, Date_sans,
                           Ligne_r_normal);

  if (Resultat < 0 || Resultat != (int)TAILLE_Finale) {
    Erreur_STR = std::string("sendto(" + std::to_string(SocketReseau) +
                             ") Err(" + std::to_string(errno) + ") : ");
    Erreur_STR = Erreur_STR + std::string(strerror(errno));
    // Erreur_STR = std::string(strerror(errno));
    cpinti_dbg::CPINTI_DEBUG(
        " [ERREUR] Probleme de transmission. Raison:'" + Erreur_STR + "'",
    ...
    if (Resultat < 0) {
      ...
      case EHOSTUNREACH: {
        ...
        cpinti_dbg::CPINTI_DEBUG(
            " [ERREUR] Hote introuvable. '" + Erreur_STR + "'",
            " [ERROR] Host not found '" + Erreur_STR + "'", "", "", Ligne_saute,
  ...
  /******************************************************************/
  // Receptionner la reponse
  cpinti_dbg::CPINTI_DEBUG("Attente d'une reponse", "Waiting for a response",
                           "net_ping", "ping()", Ligne_reste, Alerte_action,
                           Date_avec, Ligne_r_normal);
  ...
  while (current_time <= (current_time_init + 1) + (time_t)Delai_ms) {
    selectTrouve = select(SocketReseau + 1, &FD_Lecture, NULL /*&FD_Ecriture*/,
                          NULL, &IntervalleTemps);
    if (selectTrouve != 0)
  ...
  if (selectTrouve == 0) {
    cpinti_dbg::CPINTI_DEBUG("Pas de reponse. ", "No response.", "", "",
                             Ligne_saute, Alerte_avertissement, Date_sans,
                             Ligne_r_normal);
  ...
  long Temps_total =
      cpinti::Func_Cpinti::Comparer_Temps(&Temps_Actuel, &Temps_Depart);
  ...
  cpinti_dbg::CPINTI_DEBUG(
      " Temps:" + Temps_total_STR + " ms.", " Time:" + Temps_total_STR + " ms.",
