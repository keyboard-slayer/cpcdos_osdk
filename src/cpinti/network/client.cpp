/*
        ...
        Description
                Module permettant de creer un client TCP/UDP multithread multi
   I/O
        ...
        Mise a jour
                ...
                13-AOU-2020 : CORRECTION telechargement fichiers binaire
   (mauvaise condition) ~530 21-MAR-2020 : Corrections pour les fichiers sans le
   header http "Content-Length:" 12-MAR-2020 : Readaptation et correction pour
   la beta 1.2 11-07-2019	: REFONTE COMPLETE 07-12-2017	: AMELIORATION
   du code en suivant une procedure sticte de GCC 14-02-2017	: Revue des
   includes et reception + envoie via le buffer cpcdos OK! ;)
...
std::string Resolution_DNS(std::string NomAdresse) {
  // Cette fonction permet de traduire une nom de domaine DESTINATION en adresse
  // IP
...
std::string get_http_element(const char* element)
{
        ...
        // Tout mettre en MAJ
        std::for_each(element_STR.begin(), element_STR.end(), [](char & c){ c =
::toupper(c); });

        if(Request_content.find(element_STR) != std::string::npos)
        {
                std::string str_temp =
Request_content.substr(Request_content.find(element_STR) +
element_STR.length()); str_temp = str_temp.substr(0, str_temp.find("\r\n"));
...
Request_content + "-- END --\r\n", "Header HTTP : \r\n" + Request_content + "--
END --\r\n", "HTTP", "get_content()", Ligne_saute, Alerte_ok, Date_avec,
Ligne_r_normal);

                        // Tout mettre en MAJ
                        std::for_each(Request_content.begin(),
Request_content.end(), [](char & c){ c = ::toupper(c); });
...
long Demarrer_client(std::string AdresseIP, unsigned long _NumPort,
                     unsigned long __NumeroID, long __TYPE_CLIENT) {
  // Cette fonction permet de creer un client
  //  _TYPE_CLIENT 	= TCP:1 / UDP:2
  //	AdresseIP		= AdresseIP de l'hote
  //  NumPort			= Numero de port
  //	_NumeroID_STR		= Numero d'itentification unique associe a un
  // processus
  ...
  cpinti_dbg::CPINTI_DEBUG("Configuration du socket ", "Socket configuration ",
                           "CLT:" + _NumeroID_STR, "Demarrer_client()",
  ...
  cpinti_dbg::CPINTI_DEBUG(
      "Tentative de resolution du nom '" + AdresseIP + "' ... ",
      "Attempt to resolve the name '" + AdresseIP + "' ... ", "DNS",
      "Demarrer_client()", Ligne_reste, Alerte_action, Date_avec,
      Ligne_r_normal);
  ...
  if (DNS_Resultat.compare("#ERR") == 0) {
    cpinti_dbg::CPINTI_DEBUG(
        "[ERREUR] Impossible de resoudre le nom de serveur\n Raison '" +
            Erreur_STR + "'",
  ...
  cpinti_dbg::CPINTI_DEBUG("Connexion au serveur '" + AdresseIP + ":" +
                               NumPort_STR + "' (" + AdresseIP_DNS + ") ... ",
                           "Connection to server '" + AdresseIP + ":" +
                               NumPort_STR + "' (" + AdresseIP_DNS + ") ... ",
                           "CLT:" + _NumeroID_STR, "Demarrer_client()",
                           Ligne_reste, Alerte_action, Date_avec,
                           Ligne_r_normal);
  ...
  if (connect(SocketReseau, (const sockaddr *)&servaddr, sizeof(servaddr)) !=
      0) {
    Erreur_STR = std::string(strerror(errno));
    cpinti_dbg::CPINTI_DEBUG(
        "[ERREUR] Impossible de se connecter au serveur '" + AdresseIP + ":" +
            NumPort_STR + "' Raison:'" + Erreur_STR + "'",
        "[ERROR] Unable connect to server '" + AdresseIP + ":" + NumPort_STR +
            "' Reason:'" + Erreur_STR + "'",
        "", "", Ligne_saute, Alerte_erreur, Date_sans, Ligne_r_normal);
  ...
  cpinti_dbg::CPINTI_DEBUG("Client TCP lance!", "TCP Client in execution!",
                           "CLT:" + _NumeroID_STR, "", Ligne_saute, Alerte_ok,
                           Date_sans, Ligne_r_normal);
  ...
  while (EN_VIE) {
    ...
    STACK_MEMOIRE_STR =
        cpinti::cpinti_GEST_BUFF(__NumeroID, _STACK_EXTRACT_POUR_SERVEUR, "");
    ...
    if (STACK_MEMOIRE_STR.length() > 0) {
      ...
      if (BUFFER == "#STOP") {
        cpinti_dbg::CPINTI_DEBUG(
            "Fermeture du client a la demande du noyau ...",
      ...
      if (BUFFER.find("#CFG_TMPFILE ") != std::string::npos) {
        ...
        cpinti_dbg::CPINTI_DEBUG(
            "Definition d'un fichier tempon memoire '" + Fichier_TEMP_STR + "'",
            "File buffer defined to '" + Fichier_TEMP_STR + "'",
            "CLT:" + _NumeroID_STR, "", Ligne_saute, Alerte_ok, Date_sans,
            Ligne_r_normal);
      ...
      if (BUFFER.find("#CFG_VAR_POURCENT ") != std::string::npos) {
        ...
        cpinti_dbg::CPINTI_DEBUG(
            "STATS : Definition variable progression en poucentage '" +
                _tmp_cpy + "'",
      ...
      if (BUFFER.find("#CFG_VAR_SPEED ") != std::string::npos) {
        ...
        cpinti_dbg::CPINTI_DEBUG(
            "STATS : Definition variable octets par secondes '" + _tmp_cpy +
                "'",
      ...
      if (BUFFER.find("#CFG_VAR_SIZE ") != std::string::npos) {
        ...
        cpinti_dbg::CPINTI_DEBUG(
            "STATS : Definition variable octets copies '" + _tmp_cpy + "'",
      ...
      if (BUFFER.find("#CFG_VAR_SOCKET ") != std::string::npos) {
        ...
        cpinti_dbg::CPINTI_DEBUG(
            "STATS : Definition variable socket'" + _tmp_cpy + "'",
      ...
      if (BUFFER.find("#SRV ") != std::string::npos) {
        ...
        cpinti_dbg::CPINTI_DEBUG(
            "Envoi de " + BUFFER_length_STR + " octets au serveur [" +
                AdresseIP + ":" + NumPort_STR + "] Socket #" +
                SocketReseau_STR + " ... ",
            "Sending " + BUFFER_length_STR + " bytes to [" + AdresseIP + ":" +
                NumPort_STR + "] Socket #" + SocketReseau_STR + " ... ",
            "CLT:" + _NumeroID_STR, "", Ligne_reste, Alerte_action, Date_avec,
            Ligne_r_normal);

        send(SocketReseau, BUFFER.c_str(), (ssize_t)BUFFER.length(),
             CPC_MSG_DONTWAIT);
    ...
    if ((Resultat < 0) && (errno != EINTR)) {
      ...
      cpinti_dbg::CPINTI_DEBUG(
          ...
          "CLT:" + NumPort_STR, "Demarrer_serveur()", Ligne_saute,
    ...
    if (FD_ISSET(SocketReseau, &FD_socket)) {
      ...
      if (Simple_TrameHTTP == true) {
        ...
        if ((TailleContenu > 0) ||
            ((Fichier_TEMP_STR != "") && (Fichier_TEMP_FERME == false))) {
          ...
          if (Fichier_TEMP_STR != "") {
            cpinti_dbg::CPINTI_DEBUG(
                "Creation du fichier '" + Fichier_TEMP_STR + "' ...",
                "Creating file '" + Fichier_TEMP_STR + "' ...",
                "CLT:" + _NumeroID_STR, "", Ligne_reste, Alerte_action,
                Date_avec, Ligne_r_normal);

            FD_fichiertemp = fopen(Fichier_TEMP_STR.c_str(), "wb");
            buffer_recu = (char *)calloc(strlen(Fichier_TEMP_STR.c_str()) + 16,
                                         sizeof(char));
            ...
            cpinti_dbg::CPINTI_DEBUG("[OK]", "[OK]", "", "", Ligne_saute,
                                     Alerte_ok, Date_sans, Ligne_r_normal);
            // EXCEPTION !!
            cpinti::cpinti_GEST_BUFF(__NumeroID, _STACK_STOCKER_POUR_CPCDOS,
                                     "#TCP " + AdresseIP + ":" + NumPort_STR +
                                         " TCP#" + SocketReseau_STR + "=" +
                                         std::string(buffer_recu));
            doevents(20000);
          } else
            buffer_recu =
                (char *)calloc((size_t)TailleContenu + 64, sizeof(char));
          ...
          while ((octets_recu = recv(SocketReseau, buffer, 8192, 0))) {
            ...
            if (Fichier_TEMP_STR != "") {
              if (cpinti_dbg::DEBUG_ENABLED == true) {
                ...
                cpinti_dbg::CPINTI_DEBUG(
                    "Ecriture '" + buffer_STR + "' taille:" + octets_recu_STR,
            ...
            if (cpinti_dbg::DEBUG_ENABLED == true) {
              cpinti_dbg::CPINTI_DEBUG(
                  std::to_string(Octets) + " octets recu sur " +
                      Taille_Contenu_STR,
                  std::to_string(NombreOctets) + " received bytes on " +
                      Taille_Contenu_STR,
                  "CLT:" + _NumeroID_STR, "", Ligne_saute, Alerte_ok, Date_sans,
                  Ligne_r_normal);
            }
            if (true) {
              ...
              // if(true)
              {
                ...
                STACK_MEMOIRE_STR = cpinti::cpinti_GEST_BUFF(
                    __NumeroID, _STACK_EXTRACT_POUR_SERVEUR, "");
                if (STACK_MEMOIRE_STR == "#STOP") {
                  cpinti_dbg::CPINTI_DEBUG(
                      "Fermeture du client en telechargement a la demande du "
                      "noyau ...",
                      "Closing client during downloading at the request of the "
                      "kernel ...",
                      "", "", Ligne_saute, Alerte_ok, Date_sans,
                      Ligne_r_normal);
                ...
                if ((var_progression == true) &&
                    (strlen(VAR_PROGRESSION) > 1)) {

                  valeur = ((double)NombreOctets / (double)TailleFichier) * 100;
                  sprintf(_Commande_CpcdosCP, "SET/ %s = /F:CPC.INT(%f)",
                          VAR_PROGRESSION, valeur);
                  cpc_CCP_Exec_Commande(_Commande_CpcdosCP, 5);
                }
              ...
              if ((var_speed == true) && (strlen(VAR_SPEED) > 1)) {
                ...
                TempsPasse =
                    (unsigned int)((TempsFin - TempsDebut) / CLOCKS_PER_SEC);
          ...
          if (Fichier_TEMP_STR != "") {
            cpinti_dbg::CPINTI_DEBUG("Fermeture fichier", "Fermeture fichier",
                                     "CLT:" + _NumeroID_STR, "", Ligne_saute,
                                     Alerte_action, Date_avec, Ligne_r_normal);
            ...
            if (EN_VIE == true) {
              // Fermer a 100% Car il est possible que les loop se breakent
              // qu'au 100%
      ...
      if ((TailleContenu == 0) &&
          (TailleLue = read(SocketReseau, buffer, TailleBuffer) == 0)) {
        cpinti_dbg::CPINTI_DEBUG(
            "Le serveur a ferme la connexion.", "Server has close connection.",
            "CLT:" + _NumeroID_STR, "", Ligne_saute, Alerte_avertissement,
            Date_sans, Ligne_r_normal);
        EN_VIE = false;
      } else {
        ...
        // Version HTTP
        if ((*buffer == 0) ||
            ((Simple_TrameHTTP == true) && (Fichier_TEMP_STR != ""))) {
          ...
          if (_TYPE_CLIENT == 1) /* TCP */
            ...
            cpinti::cpinti_GEST_BUFF(__NumeroID, _STACK_STOCKER_POUR_CPCDOS,
                                     "#UDP " + NumPort_STR + ":" + NumPort_STR +
                                         " UDP#" + SocketReseau_STR + "=" +
                                         std::string(buffer_recu));
        } else {
