...
private:
  ...
  long Ferm_differe_SOCK[_DEFFERE_FERME]; // Tableau de socket contenant les
                                          // socket A fermer
  ...
  std::string AdresseIP_dest_STR; // Adresse IP destinataire en string (facilite
                                  // le debug)
  std::string
      AdresseIP_src_STR; // Adresse IP source en string (facilite le debug)
  std::string AdresseIP_NOM_dest_STR; // Nom domaine destinataire en string
                                      // (facilite le debug)
  std::string
      AdresseIP_NOM_src_STR; // Nom domaine source en string (facilite le debug)
...
public:
  ...
  unsigned long
  get_STATISTIQUES_Clients(); // Nombre de clients actif durant les 2 secondes
  unsigned long
  get_STATISTIQUES_Envoie(); // Nombre de paquets envoyes par secondes
  unsigned long
  get_STATISTIQUES_Reception(); // Nombre de paquets receptionnes par secondes

  std::string get_Erreur(); // Obtenir le message d'erreur

  std::string
  Resolution_DNS(const std::string Adresse); // Resoudre le nom du serveur
  ...
  unsigned long
  get_PORT_Client_uint(); // Obtenir le port du client courant (en uint)

  long
  Verifier_descripteur(long Descripteur); // Savoir s'il le descripteur est bon
  bool Verifier_correspondance(
      long Descripteur); // Savoir si le descripteur correspond
  ...
  // Fermer apres un laps de temps pour garantir la reception des donnees avant
  // fermeture
...
public:
  ...
  static bool TestAdresseIPv4(
      const std::string &Adresse); // Tester la forme IPv4 si respectee ou non
};
} // namespace NP_cpinti_socket
